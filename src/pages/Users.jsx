...
export default class Users extends Component {
    ...
    async removeUser(user){
        ...
        await axios.delete(url).then(() => {
            ...
            toast.error((<div className="centerInline"><Clear className="marginRight"></Clear>{error.response.data || 'Ocorreu um erro desconhecido, se persistir reporte'}</div>), {autoClose: 3000, closeOnClick: true})
        })
    ...
    render(){
        return (
            <Container>
                ...
                <Header title="Usuários" description="Usuários do sistema" icon="people"></Header>
                <Container className="hudBar">
                    <Grid item className="hudBarChild">
                        <Link to="/user" className="linkRouter"><CustomButton color="default" icon="add_circle_outline" text="Novo Usuário" /></Link>
                    </Grid>
                    <Grid item className="hudBarChild">
                        <SearchBar id="search_field" className="marginLeft" placeholder="Pesquisar" value={this.state.query} onChange={(query) => this.changeQueryValue(query)} onCancelSearch={() => this.changeQueryValue('')} />
                    </Grid>
                </Container>
                ...
                {!this.state.loading && this.state.users.length === 0 && <Container className="center"><p className="defaultFontColor">Ops, Nenhum resultado encontrado</p></Container>}
                {this.state.users.length > 0 && !this.state.loading && <Paper>
                <Container className="wrapper">
                    <Table className="defaultTable">
                        <TableHead>
                            <TableRow>
                                <TableCell><span className="centerVertical"><Icon fontSize="small">person</Icon>Nome</span></TableCell>
                                <TableCell><span className="centerVertical"><Icon fontSize="small">alternate_email</Icon>E-mail</span></TableCell>
                                <TableCell><span className="centerVertical"><Icon fontSize="small">bookmarks</Icon>Tipo</span></TableCell>
                        ...
                        {this.state.users.map(user => (
                            <TableRow key={user._id}>
                                ...
                                <TableCell scope="tagAdmin">
                                    {user.tagAdmin ? <CustomChip size="small" className="chipTypeUser" color="default" sizeIcon="small" icon="supervisor_account" text="Administrador"/> : <CustomChip size="small" className="chipTypeUser" color="gray" sizeIcon="small" icon="person" text="Autor"/>}
                                </TableCell>
                                <TableCell scope="_id">
                                    <CustomIconButton icon="edit" aria-label="Edit" color="default" tooltip="Editar" onClick={this.goTo(`edit-user/${user._id}`)}/>
                                    <CustomIconButton icon="delete_forever" aria-label="Delete" color="danger" tooltip="Remover" onClick={() => {
                                        this.setState({
                    ...
                    >
                        ...
                        <DialogActions>
                            ...
                            await this.removeUser(this.state.userSelected)
